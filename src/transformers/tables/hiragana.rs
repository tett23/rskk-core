...
pub fn convert(current_buffer: &str, character: char) -> Option<Vec<BufferPair>> {
  ...
  let pairs = match (current_buffer, character) {
    ...
    ("dh", 'a') => Some(vec![("でゃ", Stop)]),
    ("dh", 'i') => Some(vec![("でぃ", Stop)]),
    ("dh", 'u') => Some(vec![("でゅ", Stop)]),
    ("dh", 'e') => Some(vec![("でぇ", Stop)]),
    ("dh", 'o') => Some(vec![("でょ", Stop)]),
    ("d", 'y') => Some(vec![("dy", Continue)]),
    ("dy", 'a') => Some(vec![("ぢゃ", Stop)]),
    ("dy", 'i') => Some(vec![("ぢぃ", Stop)]),
    ("dy", 'u') => Some(vec![("ぢゅ", Stop)]),
    ("dy", 'e') => Some(vec![("ぢぇ", Stop)]),
    ("dy", 'o') => Some(vec![("ぢょ", Stop)]),
